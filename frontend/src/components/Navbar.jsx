  ...
  return (
    <nav className="border-b bg-white dark:bg-gray-900 px-4 py-2 fixed w-full z-50">
      ...
      {isAdmin ? (
        ...
        <div className="hidden md:flex justify-between items-center max-w-7xl mx-auto gap-4">
          {/* Left - Logo and Links */}
          <div className="flex items-center gap-6">
            <Link to="/" className="flex items-center gap-2 flex-shrink-0 hover:opacity-80 transition">
              <img src={logo} alt="Famazon Logo" className="h-12 w-auto" />
              <span className="ml-2 hidden sm:inline text-2xl font-bold text-gray-900 dark:text-white">Famazon</span>
            </Link>
            {/* Navigation Links */}
            <div className="hidden lg:flex items-center gap-5 text-sm text-gray-700 dark:text-gray-300">
              <Link to="/eletronics" className="hover:font-bold transition">Electronics</Link>
              <Link to="/mobile" className="hover:font-bold transition">Mobile</Link>
              <Link to="/homeandkitchen" className="hover:font-bold transition">Kitchen & Home</Link>
              <Link to="/fashion" className="hover:font-bold transition">Fashion</Link>
            </div>
          </div>

          {/* Center search bar */}
          <div className="flex flex-grow max-w-md mx-2">
            <input
              type="text"
              ...
              className="flex-grow rounded-l-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 p-2 focus:outline-none text-sm"
              aria-label="Search Famazon"
            />
            <button
              onClick={executeSearch}
              className="bg-yellow-400 p-2 rounded-r-md hover:bg-yellow-500 text-gray-900 dark:text-gray-100 transition"
          ...
          {/* Right icons & links */}
          <div className="flex items-center gap-4 text-gray-600 dark:text-gray-300 text-sm flex-shrink-0">
            <button className="flex items-center gap-1 hover:font-bold transition hidden sm:flex">
              <Globe size={18} /> EN
            </button>
            <Link to="/customer" className="hover:font-bold hidden lg:inline transition">Customer Service</Link>
            {userInfo && (
              <Link to="/cart" className="relative hover:opacity-80 transition">
                <ShoppingCart size={20} />
                {cartCount > 0 && (
                  <span className="absolute -top-2 -right-2 bg-yellow-400 text-black rounded-full text-xs px-1.5 font-bold">
                    {cartCount}
                  </span>
                )}
              </Link>
            ...
            {userInfo ? (
              <div className="flex items-center gap-3 border-l border-gray-300 dark:border-gray-600 pl-4">
                <span className="font-semibold hidden sm:inline">Hi, {userInfo.name.split(' ')[0]}</span>
                <Link to="/account" className="text-blue-500 hover:text-blue-600 transition hidden sm:inline">Account</Link>
                {userInfo.role === 'admin' && (
                  <Link to="/admin" className="text-blue-500 hover:text-blue-600 transition hidden sm:inline">Admin</Link>
                )}
                <button onClick={logoutHandler} className="text-red-500 hover:text-red-600 transition">
                  <LogOut size={18} />
                </button>
              </div>
            ) : (
              <Link to="/login" className="text-blue-500 hover:text-blue-600 transition">
                <User size={20} />
              </Link>
            )}
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
