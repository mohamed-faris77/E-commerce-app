...
function FashionPage() {
  ...
  const fashionBrands = [
    "Nike", "Adidas", "Levi's", "Zara", "H&M", "Gucci", "Puma", "Uniqlo",
    "Ray-Ban", "Forever 21", "New Balance", "Calvin Klein", "Louis Vuitton",
    "Gap", "Tommy Hilfiger", "Converse", "Banana Republic", "Old Navy",
    "Rolex", "Victoria's Secret", "Under Armour", "Kate Spade", "Reebok",
    "Ann Taylor", "Coach"
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pt-20">
      {/* Hero Section */}
      <div className="relative bg-pink-900 text-white py-16 mb-8 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-pink-900 to-rose-800 opacity-90"></div>
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1483985988355-763728e1935b?ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80')] bg-cover bg-center mix-blend-overlay opacity-20"></div>
        <div className="relative max-w-7xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 tracking-tight">Fashion & Style</h1>
          <p className="text-lg md:text-xl text-pink-100 max-w-2xl mx-auto">
            Explore the latest trends in clothing, footwear, and accessories from top global brands.
          </p>
        </div>
      </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm sticky top-24">
              <div className="flex justify-between items-center mb-6">
                ...
                {(filters.brand || filters.priceRange || filters.rating) && (
                  <button onClick={clearFilters} className="text-xs text-pink-600 hover:text-pink-800 dark:text-pink-400 font-medium">
                    Clear All
                  </button>
                )}
              </div>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                  {fashionBrands.map(brand => (
                    <label key={brand} className="flex items-center gap-2 cursor-pointer group">
                      ...
                      />
                      <span className={`text-sm ${filters.brand === brand ? 'text-pink-600 font-medium' : 'text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200'}`}>
                        {brand}
                      </span>
                    </label>
                  ))}
                  ...
                  {[
                    ...
                    <label key={range.value} className="flex items-center gap-2 cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 text-pink-600 border-gray-300 focus:ring-pink-500"
                      />
                      <span className={`text-sm ${filters.priceRange === range.value ? 'text-pink-600 font-medium' : 'text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200'}`}>
                        {range.label}
                      </span>
                    </label>
                  ))}
                  ...
                  {[4, 4.5, 4.8].map(rating => (
                    <label key={rating} className="flex items-center gap-2 cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 text-pink-600 border-gray-300 focus:ring-pink-500"
                      />
                      <span className={`text-sm ${filters.rating === String(rating) ? 'text-pink-600 font-medium' : 'text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200'}`}>
                        {rating}+ Stars
                      </span>
                    </label>
                  ))}
