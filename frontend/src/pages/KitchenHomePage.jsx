...
function KitchenHomePage() {
  ...
  const kitchenBrands = ["Instant Pot", "Ninja", "KitchenAid", "Cuisinart", "Dyson"];
      ...
      <div className="relative bg-orange-900 text-white py-16 mb-8 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-lg md:text-xl text-orange-100 max-w-2xl mx-auto">
            Upgrade your living space with premium appliances, cookware, and home essentials.
          </p>
        </div>
      </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm sticky top-24">
              <div className="flex justify-between items-center mb-6">
                ...
                {(filters.brand || filters.priceRange || filters.rating) && (
                  <button onClick={clearFilters} className="text-xs text-orange-600 hover:text-orange-800 dark:text-orange-400 font-medium">
                    Clear All
                  </button>
                )}
              </div>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                  {kitchenBrands.map(brand => (
                    <label key={brand} className="flex items-center gap-2 cursor-pointer group">
                      ...
                      />
                      <span className={`text-sm ${filters.brand === brand ? 'text-orange-600 font-medium' : 'text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200'}`}>
                        {brand}
                      </span>
                    </label>
                  ))}
                  ...
                  {[
                    ...
                    <label key={range.value} className="flex items-center gap-2 cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 text-orange-600 border-gray-300 focus:ring-orange-500"
                      />
                      <span className={`text-sm ${filters.priceRange === range.value ? 'text-orange-600 font-medium' : 'text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200'}`}>
                        {range.label}
                      </span>
                    </label>
                  ))}
                  ...
                  {[4, 4.5, 4.8].map(rating => (
                    <label key={rating} className="flex items-center gap-2 cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 text-orange-600 border-gray-300 focus:ring-orange-500"
                      />
                      <span className={`text-sm ${filters.rating === String(rating) ? 'text-orange-600 font-medium' : 'text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200'}`}>
                        {rating}+ Stars
                      </span>
                    </label>
                  ))}
